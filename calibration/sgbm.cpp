...
Sgbm::Sgbm(int height, int width, int downScaleParam, int dispRange, int CLRDispMax, int minDiff, int filterLimitValue, int wradiusY, int wradiusX, int P1, int P2){
  ...
  this->firstImage_grad_x = cv::Mat(cv::Mat::zeros(this->dheight, this->dwidth, CV_8UC1) );
  this->secondImage_grad_x = cv::Mat(cv::Mat::zeros(this->dheight, this->dwidth, CV_8UC1) );
...
int Sgbm::xGradientv2(Mat image, int x, int y)
{
    if(y == 0){
      return 2*image.at<uchar>(y, x-1) + 
                 image.at<uchar>(y+1, x-1) -
                  2*image.at<uchar>(y, x+1) -
                    image.at<uchar>(y+1, x+1);
    }
...
void Sgbm::removeInconsistencyv2(unsigned short ***S, int rows, int cols, int disparityRange, cv::Mat &disparityMap, int minimalDifference) {
    //unsigned short disparity = 0, minCost;
    //unsigned short disparity2 = 0, minCost2;
    unsigned short f, s, temp, temp2;
	  unsigned short indf = 0, inds = 0, indtemp = 0;
    ...
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            ...
            for (int d = 0; d < disparityRange; d++) {
              ...
              else if(s > temp2 && temp2 != f){
                s = temp2;
                inds = d;
              }
            }  
            
            if((((100 - minimalDifference) * s) < f*100) && (abs(indf - inds) > 1)){
			...
			for (d = 0; d < (dMax); d++){
			    ...
			    if(col - d >= 0){
			      ...
			      sum = 0;
					  ...
					  for (dy = -wradius_y; dy <= wradius_y; dy++) {
						  for (dx = -wradius_x; dx <= wradius_x; dx++) {						  						  
						    if(((col + dx - d) >= 0) && ((row + dy) >= 0) && ((col + dx) < imageL->cols) && ((row + dy) < imageL->rows)){						  
						      sum += calculatePixelCostOneWayBTv2(row + dy, col + dx, col + dx - d, *imageL, *imageR);
						    }
						  /*row2 = row + dy;
						  leftCol = col + dx;
						  rightCol = col + dx - d;
...
void Sgbm::calcSadCostv4(Mat *imageL, Mat *imageR, unsigned short ***finalResult, int dMax, int wradius_y, int wradius_x){
	int d, row, col;
	int sum;
	int dx, dy;
	char leftValue, rightValue, beforeRightValue, afterRightValue, rightValueMinus, rightValuePlus, rightValueMin, rightValueMax;
	...
	for (row = 0; row < (imageL->rows); row++){
		...
		for (col = 0; col < (imageL->cols); col++){
			...
			for (d = 0; d < (dMax); d++){
			    ...
			    if(col + d < (imageL->cols)){
			
			      sum = 0;
          			 
					  for (dy = -wradius_y; dy <= wradius_y; dy++) {
						  for (dx = -wradius_x; dx <= wradius_x; dx++) {						  						  
						    if(((col + dx) >= 0) && ((row + dy) >= 0) && ((col + dx + d) < imageL->cols) && ((row + dy) < imageL->rows)){						  
						      sum += calculatePixelCostOneWayBTv2(row + dy, col + dx, col + dx + d, *imageL, *imageR);
						    }
						  /*row2 = row + dy;
						  leftCol = col + dx;
...
unsigned short Sgbm::aggregateCost(int row, int col, int d, path &p, int rows, int cols, int disparityRange, unsigned short ***C, unsigned short ***A, unsigned short **minA, int P1, int P2) {
    unsigned short minPrev, prev, prevPlus, prevMinus;
    prev = minPrev = prevPlus = prevMinus = MAX_SHORT;
          ...
          for (int d = 0; d < disparityRange; ++d) {
              ...
              for (unsigned int pathu = 0; pathu < firstScanPaths.size(); ++pathu) { 
                ...
                if (row + p.rowDiff < 0 || row + p.rowDiff >= rows || col + p.colDiff < 0 || col + p.colDiff >= cols) {
                   ...
                   //minA[pathu][row][col] = std::min(A[pathu][row][col][d],minA[pathu][row][col]);
                }
              }              
           }
        }
        ...
        for (int col = 0; col < cols; ++col) {
          for (int d = 0; d < disparityRange; ++d) {
              ...
              for (unsigned int path = 0; path < firstScanPaths.size(); ++path) {   
                ...
                S[row][col][d] += aggregateCost(row, col, d, firstScanPaths[path], rows, cols, disparityRange, C, A[path], minA[path], P1, P2); 
                //if(A[path][row][col][d] < minA[path][row][col]) minA[path][row][col] = A[path][row][col][d];
                minA[path][row][col] = std::min(A[path][row][col][d],minA[path][row][col]);
              }
...
void Sgbm::calcCostPlusAggregateCosts(Mat *imageL, Mat *imageR, int rows, int cols, int disparityRange, unsigned short ***C, unsigned short ****A, unsigned short ***S, unsigned short ***minA, int P1, int P2, int wradius_y, int wradius_x, cv::Mat &disparityMap) {
    int sum;
    int dy, dx;
    ...
    unsigned short minCost = MAX_SHORT;
    unsigned short disparity = 0;
            /*for (int d = 0; d < disparityRange; d++) {                
...
void Sgbm::calcCostPlusAggregateCostsv2(Mat *imageL, Mat *imageR, int rows, int cols, int disparityRange, unsigned short ***C, unsigned short ****A, unsigned short ***S, unsigned short ***minA, int P1, int P2, int wradius_y, int wradius_x, cv::Mat &disparityMap) {
    int sum;
    int dy, dx;
    char leftValue, rightValue, beforeRightValue, afterRightValue, rightValueMinus, rightValuePlus, rightValueMin, rightValueMax;
    char beforeLeftValue, afterLeftValue, leftValueMinus, leftValuePlus, leftValueMin, leftValueMax;
    ...
    for (int row = 0; row < rows; ++row) {
        for (int col = 0; col < cols; ++col) {
          ...
          for (int d = 0; d < disparityRange; ++d) {            
            ...
            //sum = abs(imageL->at<short>(row, col) - imageR->at<short>(row, col - d));         
...
unsigned short Sgbm::calculatePixelCostOneWayBTv2(int row, int leftCol, int rightCol, const cv::Mat &leftImage, const cv::Mat &rightImage) {

    char leftValue, rightValue, beforeRightValue, afterRightValue, rightValueMinus, rightValuePlus, rightValueMin, rightValueMax;
    
    int u = leftImage.at<short>(row, leftCol);
    
    int ul = leftCol > 0 ? ((u + leftImage.at<short>(row, leftCol-1))/2) : u;
    int ur = leftCol < (rightImage.cols - 1) ? ((u + leftImage.at<short>(row, leftCol+1))/2) : u;
    int u0 = min(ul,ur); u0 = min(u0,u);
    int u1 = max(ul,ur); u1 = max(u1,u);
    
    int v = rightImage.at<short>(row, rightCol);
    
    int vl = rightCol > 0 ? ((v + rightImage.at<short>(row, rightCol-1))/2) : v;
    int vr = rightCol < (rightImage.cols - 1) ? ((v + rightImage.at<short>(row, rightCol+1))/2) : v;
...
void Sgbm::leftRightCheck(Mat &inputImage, Mat &inputImage2, Mat &imageResult, int dispMax, int dispTotal){
	//int dMin = 0, dMax = inputImage.size[2];
	int row, col;
	//int bestMatchSoFar;// = dMin;
	//int corrWindowSize = 3; //corrWindowSize = row = col from the corrWindow
  ...
  //*imageResult = Mat(inputImage->size[0], inputImage->size[1], CV_32FC1, float(0));
	...
	for (row = 0; row < inputImage.rows; row++){
		for (col = 0; col < inputImage.cols; col++){
			...
			if(col + dispTotal/3 < inputImage.cols){
			
			if(inputImage.at<uchar>(row, col) == 255){
			  imageResult.at<uchar>(row, col) = inputImage.at<uchar>(row, col);
			}
			else{			
			  if (((col + inputImage.at<uchar>(row, col)) < (inputImage.cols)) && (abs(inputImage.at<uchar>(row, col) - inputImage2.at<uchar>(row, col + inputImage.at<uchar>(row, col))) > dispMax)){			  
				  imageResult.at<uchar>(row, col) = 255;
			  }
			  else{			    
				  imageResult.at<uchar>(row, col) = inputImage.at<uchar>(row, col);
...
void Sgbm::run(Mat firstImage, Mat secondImage, Mat &disparityMap, int dispRange){  
  ...
  leftRightCheck(this->disparityMapDb, this->disparityMapDm, disparityMap, CLRDispMax, dispRange);   
...
void Sgbm::calcCostPlusAggregateCostsv3(Mat *imageL, Mat *imageR, int disparityRange, unsigned short ***C, unsigned short ****A, unsigned short ***S, unsigned short ***minA, int P1, int P2) {
    int sum;
    int dy, dx;
    int row, col, d;
    unsigned short f, s, temp, temp2;
	  unsigned short indf = 0, inds = 0, indtemp = 0;
	  int leftCol, rightCol, u, ul, ur, u0, u1, v, vl, vr, v0, v1, c0, c1;
	  unsigned short minPrev, minPrevOther, prev, prevPlus, prevMinus;
          ...
          for (d = 0; d < (disparityRange); d++){
			      ...
			      if(col + d < (imageL->cols)){
			        ...
			        sum = 0;
					   	...
					   	if((col >= 0) && (row >= 0) && ((col + d) < imageL->cols) && (row < imageL->rows)){						  
						        					        
						        leftCol = col;
						        rightCol = col + d;
						        
						        u = imageL->at<uchar>(row, leftCol);
						            
                    ul = leftCol > 0 ? ((u + imageL->at<uchar>(row, leftCol-1))/2) : u;
                    ur = leftCol < (imageR->cols - 1) ? ((u + imageL->at<uchar>(row, leftCol+1))/2) : u;
                    u0 = min(ul,ur); u0 = min(u0,u);
                    u1 = max(ul,ur); u1 = max(u1,u);
                    
                    v = imageR->at<uchar>(row, rightCol);
                    
                    vl = rightCol > 0 ? ((v + imageR->at<uchar>(row, rightCol-1))/2) : v;
                    vr = rightCol < (imageR->cols - 1) ? ((v + imageR->at<uchar>(row, rightCol+1))/2) : v;
                    v0 = min(vl, vr); v0 = min(v0, v);
                    v1 = max(vl, vr); v1 = max(v1, v);
                    
                    c0 = max(0, u - v1); c0 = max(c0, v0 - u);
                    c1 = max(0, v - u1); c1 = max(c1, u0 - v);
				    ...
				    for (unsigned int pathind = 0; pathind < firstScanPaths.size(); pathind++) { 
                
                A[pathind][row][col][d] = sum;
                
                path p = firstScanPaths[pathind];
                
                prev = minPrev = prevPlus = prevMinus = MAX_SHORT;
                
                if (row + p.rowDiff >= 0 && row + p.rowDiff < imageL->rows && col + p.colDiff >= 0 && col + p.colDiff < imageL->cols) {
                
                   prev = A[pathind][row + p.rowDiff][col + p.colDiff][d];
                   
                   if((d + 1) < disparityRange) prevPlus = A[pathind][row + p.rowDiff][col + p.colDiff][d+1];
                   
                   if((d) >= 1) prevMinus = A[pathind][row + p.rowDiff][col + p.colDiff][d-1];
                   
                   minPrev = minA[pathind][row + p.rowDiff][col + p.colDiff];  
      
                   A[pathind][row][col][d] += std::min(std::min((int)prevPlus + P1, (int)prevMinus + P1), std::min((int)prev, (int)minPrev + P2)) - minPrev;                         
                
                }
                
                S[row][col][d] += A[pathind][row][col][d];
                
                if(A[pathind][row][col][d] < minA[pathind][row][col]) minA[pathind][row][col] = A[pathind][row][col][d];
            } 
            
             //S[row][col][d] =  S[row][col][d]/ firstScanPaths.size();                    
         } 
      }
  }   
}
...
void Sgbm::run(Mat firstImage, Mat secondImage, int dispRange){  
  ...
  setInitialConditionv2(firstImage, secondImage, firstImage_grad_x, secondImage_grad_x, dispRange);
  ...
  //calcSadCostv4(&firstImage_grad_x, &secondImage_grad_x, C, dispRange, wradiusY, wradiusX);
 
  //aggregateCostsv3(firstImage.rows, firstImage.cols, dispRange, C, A, S, minA, P1, P2);
  
  //calcCostPlusAggregateCostsv2(&firstImage_grad_x, &secondImage_grad_x, firstImage.rows, firstImage.cols, dispRange, C, A, S, minA, P1, P2, wradiusY, wradiusX, this->disparityMapDb);
  
  calcCostPlusAggregateCostsv3(&firstImage_grad_x, &secondImage_grad_x, dispRange, C, A, S, minA, P1, P2);
   
  computeDisparity(S, firstImage.rows, firstImage.cols, dispRange, this->disparityMapDb); //Db
         
  removeInconsistencyv2(S, firstImage.rows, firstImage.cols, dispRange, this->disparityMapDb, minDiff);
   
  computeDisparityDm(S, this->disparityMapDb, firstImage.rows, firstImage.cols, dispRange, this->disparityMapDm); //Dm
   
  leftRightCheck(this->disparityMapDb, this->disparityMapDm, this->disparityMap, CLRDispMax, dispRange);   
...
void Sgbm::setInitialConditionv2(Mat &firstImage, Mat &secondImage, Mat &firstImage_grad_x, Mat &secondImage_grad_x, int dispRange){
  for (int row = 0; row < firstImage.rows; ++row) {
    for (int col = 0; col < firstImage.cols; ++col) {
      //this->disparityMap.at<uchar>(row, col) = 0;
      //firstImage_grad_x.at<short>(row,col) = 50;
      //secondImage_grad_x.at<short>(row,col) = 50;
      for (unsigned int d = 0; d < dispRange; ++d) {
        S[row][col][d] = 0;               
        for(int path = 0; path < PATHS_PER_SCAN; ++path) {
          A[path][row][col][d] = 0;
...
void Sgbm::preProcess(Mat &firstImage, Mat &secondImage, Mat &firstImage_grad_x, Mat &secondImage_grad_x){
  ...
  //firstImage_grad_x.at<short>(0,0) = firstImage_grad_x.at<short>(firstImage.rows-1,0)
  //= 

  for(int y = 0; y < (firstImage.rows); y++){
    for(int x = 0; x < (firstImage.cols); x++){
    
        if(x == 0 || x == (firstImage.cols - 1)){
          firstImage_grad_x.at<uchar>(y,x) = filterLimitValue; 
          secondImage_grad_x.at<uchar>(y,x) = filterLimitValue; 
        }
        else{
          first_image_temp_grad_x = xGradientv2(firstImage, x, y);
          second_image_temp_grad_x = xGradientv2(secondImage, x, y);  
        }                    
        ...
        firstImage_grad_x.at<uchar>(y,x) = first_image_temp_grad_x + filterLimitValue;
        secondImage_grad_x.at<uchar>(y,x) = second_image_temp_grad_x + filterLimitValue;
...
void Sgbm::runAB(Mat &firstImage, Mat &secondImage){
  ...
  resize(firstImage,downFirstImage,Size(firstImage.cols/downScaleParam,firstImage.rows/downScaleParam));
  resize(secondImage,downSecondImage,Size(secondImage.cols/downScaleParam,secondImage.rows/downScaleParam));
